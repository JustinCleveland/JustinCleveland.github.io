...
Code below reads and merges datasets such that each brewery will now contain produced beer 
It will then print the  first and last 6 observations to verify that it worked correctly.
...
dataset %>% ggplot(aes()) +geom_histogram(aes(x = dataset$ABV), binwidth = 0.005)
```
